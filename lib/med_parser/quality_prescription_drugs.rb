module MedParser
  ...
  class QualityPrescriptionDrugs
    ...
    def parse!
      product_links = @current_page.
        search('li a.products').
        collect{ |a| "#{@host}#{a['href']}" }
      product_links.uniq!

      product_links.each do |product_link|
        load(product_link)
        @current_page.search('#products').each do |row|
          attrs = {}
          row.search('.Floatleft').each_with_index do |cell, index|
            ...
            when 1 then
              attrs[:name] = @current_page.search('.selected').first.content
            when 2 then
              # Skip this cell
            when 3 then
          ...
          attrs[:url] = "#{@host}#{row.search('.right a').first['href']}"
